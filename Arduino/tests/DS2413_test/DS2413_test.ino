...
byte state = read();
  if (state == -1)Serial.println(F("Failed reading the DS2413\n"));
  if ((state&IOA)==0x1) Serial.println(F("IOA:ON\n")); else Serial.println(F("IOA:OFF\n"));
  if ((state&IOB)==0x2) Serial.println(F("IOB:ON\n")); else Serial.println(F("IOB:OFF\n"));
  /* Write 
  bool ok = false;
  ok = write(0x3);
