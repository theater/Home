...
int ICACHE_FLASH_ATTR cgiMqtt(HttpdConnData *connData) {
	int len;
		...
		len=httpdFindArg(connData->postBuff, "ESP8266cb", buff, sizeof(buff));
		if (len!=0) {
			currMqttState=atoi(buff);
			if(currMqttState==0) {
//			PostMQTT...
			INFO("STATE CHANGED to OFF\r\n");
			} else {
//			PostMQTT...
			INFO("STATE CHANGED to ON\r\n");
			}
		}
		httpdRedirect(connData, "mqtt.tpl");
		return HTTPD_CGI_DONE;
	}
...
void ICACHE_FLASH_ATTR tplMqtt(HttpdConnData *connData, char *token, void **arg) {
	...
	os_strcpy(buff, "Unknown");
	if (os_strcmp(token, "ESP8266cb_status")==0) {
		if (currMqttState) {
				os_strcpy(buff, "OPEN");
			} else {
			os_strcpy(buff, "CLOSED");
		}
	}
	httpdSend(connData, buff, -1);
}
