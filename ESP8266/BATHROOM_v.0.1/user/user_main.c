...
void ICACHE_FLASH_ATTR mqttConnectedCb(uint32_t *args)
{
	...
	MQTT_Subscribe(&mqttClient, PIN_GPIO13_TOPIC, 0);
...
// GPIO13 handling here
	if (strcmp(topicBuf,PIN_GPIO13_TOPIC)==0) {
		if (strcmp(dataBuf,"OFF")==0) {
		GPIO_OUTPUT_SET(PIN_GPIO13, 0);
		INFO("GPIO13 set to OFF\r\n");
		MQTT_Publish(&mqttClient,PIN_GPIO13_TOPIC_CB,"OFF",3,0,0);
		currGPIO4State=0;
	} else if (strcmp(dataBuf,"ON")==0) {
		GPIO_OUTPUT_SET(PIN_GPIO13, 1);
		INFO("GPIO13 set to ON\r\n");
		MQTT_Publish(&mqttClient,PIN_GPIO13_TOPIC_CB,"ON",2,0,0);
		currGPIO13State=1;
		}
	}
...
int ICACHE_FLASH_ATTR ds18b20()
{
	...
	if(SignBit){
		...
		MQTT_Publish(&mqttClient,"BR_BOILER_TEMP_SENSOR",sTemperature,6,0,0);
	} else {
		os_sprintf(sTemperature, "%d.%d", Whole, Fract < 10 ? 0 : Fract );
		MQTT_Publish(&mqttClient,"BR_BOILER_TEMP_SENSOR",sTemperature,5,0,0);
	}
	return r;
}
